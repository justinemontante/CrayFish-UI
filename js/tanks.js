...
let growoutData = (() => {
    ...
    // Default: Stocking Date May 6, 2026, Day 0
    return {
        initialStock: 68,
        mortalityHistory: [],
        stockingDate: '2026-05-06',
...
function renderGrowout() {
    ...
    document.getElementById('go-suggested-samples').textContent = suggested;
    document.getElementById('go-sampling-prompt').innerHTML = `Target: Catch at least <strong>${suggested}</strong> random samples (10%). Actual count may vary.`;

    const countInput = document.getElementById('go-sample-count');
    if (countInput.value === '' || countInput.value === '0') countInput.value = suggested;
